...
<body>
	<%
		...
		else{ //권한이 있는 사람이라면, 이제 넘어온값들을 비교해본다.
			
			if (request.getParameter("wordsEng") == null || request.getParameter("wordsKor") == null || request.getParameter("wordsContent") == null
				||request.getParameter("wordsEng").equals(" ")|| request.getParameter("wordsKor").equals(" ")||request.getParameter("wordsContent").equals(" "))
			{
				PrintWriter script = response.getWriter();
				script.println("<script>");
				...
				int result = bbsDAO.update(wordsNo, request.getParameter("wordsEng"), request.getParameter("wordsKor"),
						request.getParameter("wordsContent"));
